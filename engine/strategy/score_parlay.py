...
- 官方波胆赔率（crs_odds）大部分日期未抓到 → 无官方赔率时用
  "1/p × 0.55" 模拟（模型比分概率高估，打 55 折近似真实赔率），标注模拟赔率。
- 比分命中率极低（top1 约 10-13%），串起来期望几乎必然为负 →
  定位娱乐串，注额 ¥2/注，不推荐重注。

设计：
- 胆材：每场修正后 top1 比分，概率 ≥ min_prob(0.12)（太低没串的意义）
- 串法：2串1 全组合按概率排序取 top_n；3串1 只出 top 组合
- 概率 = 各腿修正概率连乘（模型口径，未校准，页面标注）
- 赔率 = 官方 crs_odds 优先，否则模拟
- 注额 = ¥2/注（彩票票定位）
"""
...
class ScoreParlayBuilder:
    ...
    def build(self, predictions: list[dict]) -> list[ScoreTicket]:
        ...
        top = pool[: max(len(pool), 2)]
        for l1, l2 in combinations(pool[:6], 2):
            hit = l1.prob * l2.prob
        ...
        if len(pool) >= 3:
            ...
            tickets.append(ScoreTicket(
                ...
                hit_prob=hit, stake=STAKE_UNIT, potential=round(STAKE_UNIT * odds, 2),
                odds_source=src,
                note=f"三场比分全中才赢（赔率 {odds:.1f} 倍）",
            ))

        tickets.sort(key=lambda t: t.hit_prob, reverse=True)
